...
export default function SurveyResponsesModal(props: {
  isOpen: boolean;
  onClose: () => void;
  surveyId: string | null;
  surveyTitle?: string;
}) {
  const { isOpen, onClose, surveyId, surveyTitle } = props;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const baseUrl =
        ...
        "http://localhost:5000";
      const endpoint = `${baseUrl.replace(/\/+$/, "")}/responses/survey/${encodeURIComponent(
        surveyId
      )}`;
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `survey-responses-${surveyId || "unknown"}.csv`;
    a.click();
                          ...
                          <button
                            ...
                            onClick={() =>
                              setExpandedId((current) =>
                                current === (r._id || r.id) ? null : (r._id || r.id || `${idx}`)
                              )
                            }
