...
import { AdminSurvey, fetchAdminSurveys } from "@/lib/adminApi";

const getId = (s: AdminSurvey) => s._id || s.id || `${s.user || "unknown"}:${s.title || ""}`;
...
export default function AdminSurveysPage() {
  ...
  const load = useCallback(async () => {
    ...
    const data = res.data as { surveys?: AdminSurvey[] } | AdminSurvey[];
  ...
  const filtered = useMemo(() => {
    ...
    return surveys.filter((s) => {
      ...
      return title.includes(q) || desc.includes(q) || status.includes(q) || user.includes(q);
    });
  }, [surveys, search]);

  const totals = useMemo(() => {
    const totalResponses = surveys.reduce((sum, s) => sum + Number(s.responses ?? 0), 0);
    const active = surveys.filter((s) => (s.status || "").toLowerCase() === "active").length;
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Search by title, status, userId..."
